...
if (1)
    ...
    Ntotal = N;
    N1 = 1; N2 = N;
    save('./results/RDSA_fig7.mat','-v7.3');
    ...
    str = sprintf('./results/RDSA_fig7_org');
    saveas(pic,[str,'.fig']);
    set(gca, 'FontSize', 16);
    b=get(gca);
...
if (1)
    ...
    for cnt = 1:numel(vec)
        ...
        str = sprintf('./results/RDSA_fig7_comp_bw_%d',cntb);
        saveas(pic,[str,'.fig']);
        set(gca, 'FontSize', 16);
        b=get(gca);
        set(b.XLabel, 'FontSize', 16);set(b.YLabel, 'FontSize', 16);set(b.ZLabel, 'FontSize', 16);set(b.Title, 'FontSize', 16);
        ...
        pbaspect([10 1 1]); set(pic, 'Position', [200, 200, 1200, 200]);%axis([N1/Ntotal*480, N2/Ntotal*480,-5,5]);
        str = sprintf('./results/RDSA_fig7_comp_res_%d',cntb);
        saveas(pic,[str,'.fig']);
        set(gca, 'FontSize', 16);
        b=get(gca);
        set(b.XLabel, 'FontSize', 16);set(b.YLabel, 'FontSize', 16);set(b.ZLabel, 'FontSize', 16);set(b.Title, 'FontSize', 16);
...
if (1)
    ...
    tit = sprintf('./results/RDSA_fig7_shape_%d.fig',0);
    saveas(pic,tit);
    str = sprintf('./results/RDSA_fig7_shape_%d',0);
    print(gcf, '-depsc', str);   command = sprintf('epstopdf %s.eps',str);      system(command);
    ...
    b=get(gca);
    set(b.XLabel, 'FontSize', 32);set(b.YLabel, 'FontSize', 32);set(b.ZLabel, 'FontSize', 32);set(b.Title, 'FontSize', 32);
    tit = sprintf('./results/RDSA_fig7_shapeCn_%d.fig',1);
    saveas(pic,tit);
    str = sprintf('./results/RDSA_fig7_shapeCn_%d',1);
    print(gcf, '-depsc', str);   command = sprintf('epstopdf %s.eps',str);      system(command);
    ...
    b=get(gca);
    set(b.XLabel, 'FontSize', 32);set(b.YLabel, 'FontSize', 32);set(b.ZLabel, 'FontSize', 32);set(b.Title, 'FontSize', 32);
    tit = sprintf('./results/RDSA_fig7_shapeSn_%d.fig',1);
    saveas(pic,tit);
    str = sprintf('./results/RDSA_fig7_shapeSn_%d',1);
    print(gcf, '-depsc', str);   command = sprintf('epstopdf %s.eps',str);      system(command);
