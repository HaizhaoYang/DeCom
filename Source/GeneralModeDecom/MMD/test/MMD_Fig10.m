...
if (0)
    ...
    Ntotal = N;
    N1 = N/8; N2 = N/2;
    save('./results/MMD_fig10.mat','-v7.3');
    ...
    str = sprintf('./results/MMD_fig10_org');
    saveas(pic,[str,'.fig']);
    set(gca, 'FontSize', 16);
    b=get(gca);
...
if (0)
    ...
    for cnt = 1:numel(vec)
        ...
        str = sprintf('./results/MMD_fig10_comp_bw_%d',cntb);
        saveas(pic,[str,'.fig']);
        set(gca, 'FontSize', 16);
        b=get(gca);
        set(b.XLabel, 'FontSize', 16);set(b.YLabel, 'FontSize', 16);set(b.ZLabel, 'FontSize', 16);set(b.Title, 'FontSize', 16);
        ...
        pbaspect([10 1 1]); set(pic, 'Position', [200, 200, 1200, 200]);%axis([N1/Ntotal*480, N2/Ntotal*480,-5,5]);
        str = sprintf('./results/MMD_fig10_comp_res_%d',cntb);
        saveas(pic,[str,'.fig']);
        set(gca, 'FontSize', 16);
        b=get(gca);
        set(b.XLabel, 'FontSize', 16);set(b.YLabel, 'FontSize', 16);set(b.ZLabel, 'FontSize', 16);set(b.Title, 'FontSize', 16);
...
if (1)
    ...
    tit = sprintf('./results/MMD_fig10_shape_%d.fig',0);
    saveas(pic,tit);
    str = sprintf('./results/MMD_fig10_shape_%d',0);
    print(gcf, '-depsc', str);   command = sprintf('epstopdf %s.eps',str);      system(command);
    ...
    b=get(gca);
    set(b.XLabel, 'FontSize', 32);set(b.YLabel, 'FontSize', 32);set(b.ZLabel, 'FontSize', 32);set(b.Title, 'FontSize', 32);
    tit = sprintf('./results/MMD_fig10_shapeCn_%d.fig',1);
    saveas(pic,tit);
    str = sprintf('./results/MMD_fig10_shapeCn_%d',1);
    print(gcf, '-depsc', str);   command = sprintf('epstopdf %s.eps',str);      system(command);
    ...
    b=get(gca);
    set(b.XLabel, 'FontSize', 32);set(b.YLabel, 'FontSize', 32);set(b.ZLabel, 'FontSize', 32);set(b.Title, 'FontSize', 32);
    tit = sprintf('./results/MMD_fig10_shapeSn_%d.fig',1);
    saveas(pic,tit);
    str = sprintf('./results/MMD_fig10_shapeSn_%d',1);
    print(gcf, '-depsc', str);   command = sprintf('epstopdf %s.eps',str);      system(command);
