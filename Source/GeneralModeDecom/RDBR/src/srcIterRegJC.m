function [shape,component,errorRec,SL2Rec,iter,flag] = srcIterRegJC(fff,N,numGroup,insAmplitude,insPhase,opt,fTrue,shapeTrue)
...
% Haizhao Yang, and Ingrid Daubechies. We adopt the Jacobi iterative 
% scheme.
%
% Input:
% fff       - given signal
...
%
% By Jieren Xu and Haizhao Yang, 2016
% Modified by Haizhao Yang, 2017, add comments and clean up the code
...
for cntGroup = 1:numGroup
    ...
    X = phi2(:);   Y = fff./insAmplitude(cntGroup,:); Y = Y(:);
    ...
    componentInLoop{cntGroup} = (ppval(pp{cntGroup},mod(insPhase(cntGroup,:),1))-shift).*insAmplitude(cntGroup,:);
...
while iter< maxiter && error> eps_error && delta>eps_error && ...
    ...
    for cntGroup = 1:numGroup
        % warping and folding
        phi2 = mod(insPhase(cntGroup,:), 1) ;
        X = phi2(:);    Y = f./insAmplitude(cntGroup,:); Y = Y(:);
