...
if (1)
    ...
    if (1) % test example: two components
        ...
        fTrue{2} = f2;
        numGroup = 2;
        NM = 1.5; % same as in the DSA when SNR = -3
        ns = NM*randn(1,N);
        fff = f1+ f2 + ns;
        shapeTrue = cell(1,2);
        shapeTrue{1} = @(x) sh1(x);
    ...
    save ./results/RDBR_fig9ns.mat;
end

if (1)
    load ./results/RDBR_fig9ns.mat;
    ...
    for i = 1:num_group
        ...
        tit = sprintf('./results/RDBR_fig9_%d.fig',i+2);
        saveas(pic,tit);
        str = sprintf('./results/RDBR_fig9_%d',i+2);
        print(gcf, '-depsc', str);
    end
end
