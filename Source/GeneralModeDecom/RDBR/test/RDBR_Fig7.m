% This code generate Figure 7 in the paper "Recursive Diffeomorphism-Based
...
if (1)
    ...
    save ./results/RDBR_fig7_2.mat;
end



if (1)
    load ./results/RDBR_fig7_2.mat;
    close all;
    ...
    for i = 1:num_group
        ...
        tit = sprintf('./results/RDBR_fig7_%d.fig',i+2);
        saveas(pic,tit);
        str = sprintf('./results/RDBR_fig7_%d',i+2);
        print(gcf, '-depsc', str);
    end
    
    pic = figure;
    plot(SL2Rec,'LineWidth',2);
    axis square;
    hold off;
    xlabel('j');ylabel('\epsilon_2^{(j)}');
    set(gca, 'FontSize', 16);
    b=get(gca);
    set(b.XLabel, 'FontSize', 16);set(b.YLabel, 'FontSize', 16);set(b.ZLabel, 'FontSize', 16);set(b.Title, 'FontSize', 16);
    tit = sprintf('./results/RDBR_fig7_%d.fig',5);
    saveas(pic,tit);
    str = sprintf('./results/RDBR_fig7_%d',5);
    print(gcf, '-depsc', str);
    
    pic = figure;
    plot(errorRec,'LineWidth',2);
    axis square;
    hold off;
    xlabel('j');ylabel('\epsilon_1^{(j)}');
    set(gca, 'FontSize', 16);
    b=get(gca);
    set(b.XLabel, 'FontSize', 16);set(b.YLabel, 'FontSize', 16);set(b.ZLabel, 'FontSize', 16);set(b.Title, 'FontSize', 16);
    tit = sprintf('./results/RDBR_fig7_%d.fig',6);
    saveas(pic,tit);
    str = sprintf('./results/RDBR_fig7_%d',6);
    print(gcf, '-depsc', str);
end
